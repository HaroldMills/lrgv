"""
Script that extracts Nighthawk clips from Vesper Recorder recordings.

This script is a quick hack, made by making minor modifications to
the `run_nighthawk_lrgv.py` script to only extract clips instead of
running Nighthawk and then extracting clips. We eventually want to
create two scripts that replace both `run_nighthawk_lrgv` and this
one. One script will just run Nighthawk and the other will extract
clips. The two new scripts will also support date ranges instead of
just a single date.
"""
